...
const ensureRoomActive = async (roomId) => {
...
const ensureRoomActiveBySlug = async (slug) => {
...
app.post('/rooms', async (req, res) => {
    ...
    if (recent.length >= config.roomCreationLimit)
        return res.status(429).json({ error: 'rate_limited' });
    const { type, name, durationMinutes, password, allowAttachments = true, allowLinks = true, requireApproval = false, selfDestructModeEnabled = false, burnAfterReadEnabled = false, panicButtonEnabled = false, screenshotWarningEnabled = false, tags = [], rules } = req.body;
...
app.post('/rooms/:id/join', async (req, res) => {
    ...
    if (room.requireApproval) {
        ...
        if (!pendingJoins.has(room.id))
            pendingJoins.set(room.id, new Map());
...
app.post('/rooms/:id/join-requests/:requestId/approve', requireMember, async (req, res) => {
    if (!req.member?.isCreator)
        return res.status(403).json({ error: 'forbidden' });
    if (req.member.roomId !== req.params.id)
        return res.status(403).json({ error: 'forbidden' });
...
app.post('/rooms/:id/join-requests/:requestId/deny', requireMember, async (req, res) => {
    if (!req.member?.isCreator)
        return res.status(403).json({ error: 'forbidden' });
    if (req.member.roomId !== req.params.id)
        return res.status(403).json({ error: 'forbidden' });
...
app.get('/rooms/:id/messages', requireMember, async (req, res) => {
    if (!req.member)
        return res.status(401).json({ error: 'unauthorized' });
    if (req.member.roomId !== req.params.id)
        return res.status(403).json({ error: 'forbidden' });
...
app.get('/rooms/:id/members', requireMember, async (req, res) => {
    if (!req.member)
        return res.status(401).json({ error: 'unauthorized' });
    if (req.member.roomId !== req.params.id)
        return res.status(403).json({ error: 'forbidden' });
...
app.post('/rooms/:id/timer', requireMember, async (req, res) => {
    if (!req.member?.isCreator)
        return res.status(403).json({ error: 'forbidden' });
    if (req.member.roomId !== req.params.id)
        return res.status(403).json({ error: 'forbidden' });
...
app.post('/rooms/:id/delete', requireMember, async (req, res) => {
    if (!req.member?.isCreator)
        return res.status(403).json({ error: 'forbidden' });
    if (req.member.roomId !== req.params.id)
        return res.status(403).json({ error: 'forbidden' });
...
app.post('/rooms/:id/settings', requireMember, async (req, res) => {
    if (!req.member?.isCreator)
        return res.status(403).json({ error: 'forbidden' });
    if (req.member.roomId !== req.params.id)
        return res.status(403).json({ error: 'forbidden' });
    const allowedKeys = ['allowAttachments', 'allowLinks', 'selfDestructModeEnabled', 'burnAfterReadEnabled', 'panicButtonEnabled', 'screenshotWarningEnabled', 'rules', 'tags'];
    const data = {};
...
app.post('/rooms/:id/attachments', requireMember, upload.single('file'), async (req, res) => {
    if (!req.member)
        return res.status(401).json({ error: 'unauthorized' });
    const room = await ensureRoomActive(req.params.id);
    if (!room)
        return res.status(410).json({ error: 'room_expired' });
    if (room.id !== req.member.roomId)
        return res.status(403).json({ error: 'forbidden' });
    if (!room.allowAttachments)
        return res.status(403).json({ error: 'attachments_disabled' });
    const file = req.file;
    if (!file)
...
app.get('/rooms/:id/attachments/:attachmentId', requireMember, async (req, res) => {
    if (!req.member)
        return res.status(401).json({ error: 'unauthorized' });
    if (req.member.roomId !== req.params.id)
        return res.status(403).json({ error: 'forbidden' });
    const attachment = await prisma.attachment.findUnique({ where: { id: req.params.attachmentId } });
    if (!attachment || attachment.roomId !== req.params.id)
        return res.status(404).json({ error: 'not_found' });
    res.json({
        id: attachment.id,
...
roomsNamespace.on('connection', socket => {
    ...
    socket.on('join_room', async ({ memberToken }) => {
        ...
        socket.join(room.id);
        if (!activeSessions.has(room.id))
            activeSessions.set(room.id, new Map());
    ...
    socket.on('edit_message', async (payload) => {
        ...
        if (!room)
            return;
        if (message.senderSessionId !== session.id && !session.isCreator)
            return;
    ...
    socket.on('remove_reaction', async (payload) => {
        ...
        if (!message || message.roomId !== decoded.roomId)
            return;
    ...
    socket.on('message_seen', async (payload) => {
        ...
        if (!message)
            return;
        if (message.roomId !== decoded.roomId)
            return;
        await prisma.messageReadReceipt.upsert({
            ...
            if (!existing)
                await prisma.messageReadReceipt.create({ data: { messageId, memberSessionId: decoded.memberSessionId } });
        });
        if (message.burnAfterRead && message.burnReadTargetSessionIds.length > 0) {
            ...
            const burnTargets = message.burnReadTargetSessionIds;
    ...
    socket.on('panic', async (payload) => {
        ...
        if (!decoded || decoded.roomId !== roomId)
            return;
        const room = await ensureRoomActive(roomId);
        if (!room || !room.panicButtonEnabled)
            return;
    ...
    socket.on('mute_member', async (payload) => {
        ...
        if (!target || target.roomId !== session.roomId)
            return;
